...
import { Loader2, Wallet, Check, AlertCircle, MapPin, Phone as PhoneIcon, User as UserIcon } from "lucide-react";
...
export default function Checkout() {
  ...
  const handlePay = async () => {
    ...
    try {
      const orderRes = await api.post("/payments/order", { plan_id: planId });
      const order = orderRes.data;

      if (order.mock) {
        // MOCKED Razorpay stub: we skip the checkout.js modal and verify immediately.
        // Replace with real Razorpay modal once RAZORPAY_KEY_ID/SECRET are set on backend.
        setStatus("verifying");
        ...
        // Real Razorpay flow — loads Checkout.js script on the fly
        ...
        const options = {
          ...
          handler: async (res) => {
            ...
            try {
              ...
              setStatus("error"); toast.error(e?.response?.data?.detail || "Verification failed");
            }
          },
  ...
  return (
    <div className="max-w-3xl mx-auto px-6 py-10" data-testid="checkout-page">
      <p className="text-xs tracking-overline uppercase font-bold text-secondary">Checkout</p>
      ...
      <div className="mt-8 grid md:grid-cols-5 gap-6">
        <div className="md:col-span-3 bg-card rounded-2xl border border-black/5 p-6">
          ...
          <ul className="mt-5 space-y-2 text-sm">
            <li className="flex items-center gap-2"><Check className="h-4 w-4 text-primary" /> {plan.meals} meals across {plan.duration_days} days</li>
          ...
          <div className="mt-6 border-t border-black/5 pt-5">
            ...
            <Button variant="outline" size="sm" onClick={() => navigate(`/profile?next=/checkout/${planId}`)} className="rounded-full mt-3" data-testid="edit-profile-button">Edit</Button>
          </div>
        </div>
        ...
        <div className="md:col-span-2 space-y-4">
          <div className="bg-primary text-primary-foreground rounded-2xl p-6" data-testid="wallet-preview">
            ...
            <p className="text-[10px] tracking-overline uppercase font-bold text-primary-foreground/70 mt-3">Amount loaded to wallet</p>
          ...
          <Button onClick={handlePay} disabled={submitting || status === "success"} data-testid="pay-button" className="w-full h-12 rounded-full bg-primary hover:bg-primary/90 font-semibold text-base">
            {status === "idle" && `Pay ₹${plan.amount.toFixed(0)}`}
