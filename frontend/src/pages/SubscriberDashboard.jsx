...
import { Utensils, Moon, Sun, Clock } from "lucide-react";
...
export default function SubscriberDashboard() {
  ...
  const load = async () => {
    try {
      const [s, q, m, h] = await Promise.all([
  ...
  if (loading) {
    return <div className="p-12 text-center text-muted-foreground">Loading your pass…</div>;
  }
  ...
  return (
    <div className="max-w-6xl mx-auto px-6 md:px-8 lg:px-12 py-10" data-testid="subscriber-dashboard">
      <div className="mb-10" data-testid="subscription-status">
        ...
        <h1 className="font-display font-extrabold text-3xl md:text-4xl tracking-tight mt-2">Your meal pass</h1>
      </div>
        ...
        <div className="lg:col-span-3">
          ...
          {!sub && (
            <div className="mt-6 bg-secondary/10 border border-secondary/20 rounded-2xl p-6 text-center" data-testid="no-sub-banner">
              <p className="font-display font-bold text-lg">You don't have an active pass</p>
              <p className="text-sm text-muted-foreground mt-1">Grab a plan to start checking in.</p>
              <Link to="/plans">
