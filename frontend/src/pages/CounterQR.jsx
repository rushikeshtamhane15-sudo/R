...
import { Sun, Moon, Download, ExternalLink, Copy, Maximize2 } from "lucide-react";
import { toast } from "sonner";

const LOCATION = "main";

export default function CounterQR() {
  ...
  const refresh = useCallback(async () => {
    try {
      ...
      setData(r.data);
      const left = Math.max(1, r.data.rotates_at - Math.floor(Date.now() / 1000));
      setSecondsLeft(left);
    } catch {
      toast.error("Could not load counter QR");
    }
  }, [meal]);
  ...
  // Tick countdown + auto-refresh on rotation
  useEffect(() => {
  ...
  const copyPublic = async () => {
    ...
    toast.success("Public kiosk URL copied");
  };

  const openKiosk = () => window.open(publicUrl, "_blank");

  return (
    <div className="max-w-5xl mx-auto px-6 md:px-8 lg:px-12 py-10" data-testid="counter-qr-page">
      <div className="flex flex-wrap items-end justify-between gap-4 mb-6">
        <div>
          <p className="text-xs tracking-overline uppercase font-bold text-secondary">Counter display</p>
          <h1 className="font-display font-extrabold text-3xl md:text-4xl tracking-tight mt-2">Subscribers — scan to check in</h1>
      ...
      <div className="grid lg:grid-cols-5 gap-6">
        <div className="lg:col-span-3 bg-card rounded-3xl border border-black/5 shadow-xl p-8" data-testid="counter-qr">
          <div className="flex gap-2 mb-6">
            <Button
              onClick={() => setMeal("lunch")}
              data-testid="counter-meal-lunch"
            ...
            <Button
              ...
              data-testid="counter-meal-dinner"
          ...
          <div className="flex justify-center" data-testid="counter-qr-image">
            {data ? (
              <div className="bg-white p-6 rounded-2xl border-2 border-primary/10">
                <QRCodeSVG value={data.counter_code} size={300} level="M" fgColor="#4b5c4a" />
              </div>
            ) : (
        ...
        <div className="lg:col-span-2 space-y-4">
          <div className="bg-card rounded-2xl border border-black/5 p-6" data-testid="public-url-card">
            ...
            <p className="text-xs text-muted-foreground mt-3">No login needed. Open this on the counter device.</p>
          </div>
