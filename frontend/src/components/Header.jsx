...
export default function Header() {
  ...
  if (user) {
    ...
    if (user.role === "admin" || user.role === "staff") {
      ...
      navLinks.push({ to: "/counter", label: "Counter" });
    }
  }

  return (
    <header className="sticky top-0 z-30 backdrop-blur-xl bg-[hsl(var(--background))]/80 border-b border-black/5" data-testid="app-header">
      <div className="max-w-7xl mx-auto flex items-center justify-between px-6 md:px-8 lg:px-12 py-5">
        <Link to="/" className="flex items-center gap-2.5" data-testid="logo-link">
          ...
          <div className="flex flex-col leading-none">
            <span className="font-display font-extrabold text-lg text-foreground">eFoodCare</span>
            <span className="text-[10px] tracking-overline uppercase font-bold text-secondary mt-0.5">ghar se achha khana</span>
          </div>
        </Link>
          ...
          {navLinks.map((l) => (
            <Link
              ...
              className={`px-4 py-2 rounded-lg text-sm font-semibold transition-all ${
                location.pathname === l.to ? "bg-accent text-accent-foreground" : "text-muted-foreground hover:text-foreground hover:bg-accent/60"
              }`}
            >
              {l.label}
            </Link>
        ...
        <div className="flex items-center gap-3">
          {user ? (
            <>
              ...
              <Button variant="outline" size="sm" onClick={logout} data-testid="logout-button" className="rounded-full">
                <LogOut className="h-4 w-4 mr-1.5" strokeWidth={1.75} /> Logout
              </Button>
            </>
          ) : (
            <Link to="/login" data-testid="header-login-link">
              <Button className="rounded-full bg-primary hover:bg-primary/90" data-testid="header-login-button">Sign In</Button>
            </Link>
          )}
        </div>
      </div>
